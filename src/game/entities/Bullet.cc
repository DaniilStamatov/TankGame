...
#include "Tank.h"
namespace tanks {
Bullet::Bullet(nova::Scene &scene, nova::Entity &owner, const glm::vec2 &dir,
               float damage, float speed, const glm::vec3 &pos)
    : GameObject(scene), m_owner(owner), m_damage(damage), m_speed(speed),
      m_pos(pos), m_dir(dir) {}
void Bullet::OnInit() {
...
void Bullet::OnUpdate(float dt) {
  Transform().position += glm::vec3(
      m_dir * glm::vec2(dt) * GetComponent<BulletComponent>().speed, 0.0f);
}
} // namespace tanks