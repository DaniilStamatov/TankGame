...
bool nova::InputSystem::GetKey(KeyCode key, InputState state) const {
    ...
    switch (state) {
        case InputState::Held: return current;
        case InputState::Pressed: return current && !previous;
        case InputState::Released: return !current && previous;
    }
    return false;
}
